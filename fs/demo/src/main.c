...
static void* demo_mount(struct fuse_conn_info * conn_info){
    ...
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, &disk_size);
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, &io_size);

    int block_size = 2 * io_size;

    printf("super.sz_io: %d\n", io_size);
    printf("super.sz_disk: %d\n", disk_size);
    printf("super.sz_blks: %d\n", block_size);

    /* 填充super信息 */
    super.sz_io = io_size;
    super.sz_disk = disk_size;
    super.sz_blks = block_size; 
...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    uint8_t *mem = malloc(512 * sizeof(uint8_t));
    ddriver_read(super.driver_fd, mem, super.sz_io);

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry *dentry = malloc(sizeof(struct demo_dentry));
    memcpy(dentry, mem, sizeof(struct demo_dentry));
